...
for root, dirs, files in os.walk(dataset_folder):
    for file in files:
        ...
        if file.lower().endswith(('.png', '.jpg', '.jpeg')):
            # Create a relative path to the image (relative to the Dataset folder)
            relative_path = os.path.relpath(os.path.join(root, file), dataset_folder)
            relative_path = relative_path.replace('\\', '/')  # Ensure it's forward slash
...
# Initialize the CSV with headers: id, imagePath, label_user1, label_user2, label_user3
with open(csv_file, mode='w', newline='') as file:
    ...
    for image_path, folder in training_data:
        ...
        writer.writerow([id_counter, image_path, label, label, label])
        id_counter += 1
